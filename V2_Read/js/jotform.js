        ...
        $$('.form-submit-print').each(function(print_button){
            ...
            print_button.observe("click", function(){
                ...
                $$('.form-textarea, .form-textbox').each(function(el){
                    ...
                    if(el.id.indexOf("_area") != -1 || el.id.indexOf("_phone") != -1 ){
        ...
        $$('.form-pagebreak').each(function(page, i){
            ...
            section.select('.form-pagebreak-next').invoke('observe', 'click', function(){ // When next button is clicked
                ...
                if (JotForm.validateAll(JotForm.getForm(section))) {
                    ...
                    //Kemal make nicedit nicer to pagebreaks bugfix:219657 TODO: this fix is a workaround, I could not determine what happens in the first place
                    JotForm.currentSection.select('.form-line').each(function(el){
                        var nicdetected = false;
                        el.select('.nicEdit-main').each(function(el2){nicdetected=true});
                        if(nicdetected === true){
                            var target = el.select('.form-input>div');
                            for(var tmpi=0; tmpi<target.length;tmpi++){
                                target[tmpi].style.width = "353px";
                                if(target[tmpi].select(".nicEdit-main").length !== 0)target[tmpi].select(".nicEdit-main")[0].style.minHeight = "150px";
                            }
                        }
                    });
                    //endof bugfix 219657
            ...
            $$('#'+form.id+' *[class*="validate"]').each(function(input){                
                ...
                input.validateInput = function(deep)
                {
                    ...
                    if(vals.include('disallowPast')) {
                        ...
                        var currentDate = dat.getFullYear() + "-" + month + "-" + dat.getDate();
